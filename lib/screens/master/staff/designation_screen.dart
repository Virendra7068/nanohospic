...
class _DesignationScreenState extends State<DesignationScreen> {
  List<DesignationEntity> _departmentList = [];
  List<DesignationEntity> _filteredDesignation = [];
  ...
  late DesignationRepository _designationRepository;
  Timer? _syncTimer;
  ...
  void initState() {
    ...
    _searchController.addListener(() {
      _filterDesignation(_searchController.text);
    });
  ...
  Future<void> _initializeDatabase() async {
    ...
    try {
      print('🚀 Starting department screen...');

      final db = await DatabaseProvider.database;
      _designationRepository = DesignationRepository(db.designationDao);

      try {
        final test = await _designationRepository.getAllDesignations();
        print('✅ Department table working! Records: ${test.length}');
      } catch (e) {
      ...
      print('✅ Department screen initialized successfully');
    } catch (e) {
...
''';
  ...
  Future<void> _loadLocalDepartments() async {
    try {
      final departments = await _designationRepository.getAllDesignations();
      print('Loaded ${departments.length} departments from database');
      setState(() {
        _departmentList = departments;
        _filteredDesignation = List.from(_departmentList);
      });
    } catch (e) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          await _designationRepository.syncFromServer(departmentList);
          await _loadLocalDepartments();
          await _loadSyncStats();
  ...
  Widget build(BuildContext context) {
    final displayDepartments = _isSearching
        ? _filteredDesignation
        : _departmentList;
  ...
  Widget _buildBody(List<DesignationEntity> displayDepartments) {
  ...
  Widget _buildDepartmentItem(DesignationEntity department, int index) {
    return Container(
  ...
  void _filterDesignation(String query) {
    setState(() {
      if (query.isEmpty) {
        _filteredDesignation = List.from(_departmentList);
      } else {
        _filteredDesignation = _departmentList
            .where(
              (dept) =>
...
class _AddDepartmentScreenState extends State<AddDepartmentScreen> {
  ...
  Future<void> _saveDepartment() async {
    ...
    try {
      final department = DesignationEntity(
      ...
      await _designationRepository.insertDesignation(department);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ? Center(child: CircularProgressIndicator(color: Color(0xff016B61)))
