...
class _PaymentModeScreenState extends State<PaymentModeScreen> {
  List<PaymentModeEntity> _paymentModes = [];
  List<PaymentModeEntity> _filteredPaymentModes = [];
  bool _isLoading = false;
  bool _isSyncing = false;
  String? _error;
  final TextEditingController _searchController = TextEditingController();
  bool _isSearching = false;
  late PaymentModeRepository _paymentModeRepository;
  Timer? _syncTimer;
  int _totalRecords = 0;
  ...
  Future<void> _loadLocalPaymentModes() async {
    ...
    try {
      final paymentModes = await _paymentModeRepository.getAllPaymentModes();
  ...
  Future<void> _loadSyncStats() async {
    _totalRecords = await _paymentModeRepository.getTotalCount();
    _syncedRecords = await _paymentModeRepository.getSyncedCount();
    _pendingRecords = await _paymentModeRepository.getPendingCount();
    setState(() {});
  }
  ...
  Future<void> _syncFromServer() async {
    ...
    try {
      // await _paymentModeRepository.syncFromServer();
      await _loadLocalPaymentModes();
      await _loadSyncStats();
  ...
  Future<void> _syncPendingChanges() async {
    try {
      final pendingPaymentModes = await _paymentModeRepository.getPendingSync();
      ...
      for (final paymentMode in pendingPaymentModes) {
        if (paymentMode.isDeleted == 1) {
          if (paymentMode.serverId != null) {
            ...
            await _paymentModeRepository.markAsSynced(paymentMode.id!);
          } else {
            await _paymentModeRepository.markAsSynced(paymentMode.id!);
          }
          ...
          if (serverId != null) {
            await _paymentModeRepository.updateServerId(
              paymentMode.id!,
              serverId,
            );
            await _paymentModeRepository.markAsSynced(paymentMode.id!);
  ...
  Future<int?> _addToServer(PaymentModeEntity paymentMode) async {
    try {
  ...
  Future<bool> _updateOnServer(PaymentModeEntity paymentMode) async {
    try {
      final response = await http
          .put(
            Uri.parse(
  ...
  Future<void> _addPaymentMode(String name, String description) async {
    ...
    try {
      final paymentMode = PaymentModeEntity(
        name: name,
      ...
      await _paymentModeRepository.insertPaymentMode(paymentMode);
  ...
  Future<bool> _deletePaymentMode(int paymentModeId) async {
    setState(() {
      _isLoading = true;
    });

    try {
      await _paymentModeRepository.softDeletePaymentMode(paymentModeId, 'user');
      await _loadLocalPaymentModes();
      await _loadSyncStats();
  ...
  void _showDeleteConfirmationDialog(PaymentModeEntity paymentMode) {
    showDialog(
      context: context,
                    ...
                    Expanded(
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          final success = await _deletePaymentMode(
                            paymentMode.id!,
                          );
  ...
  Widget _buildPaymentModeItem(PaymentModeEntity paymentMode, int index) {
    return Container(
